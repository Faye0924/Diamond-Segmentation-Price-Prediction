...
Due to NDA restrictions on the original sponsor dataset, this repository uses the R built-in `diamonds` dataset as a substitute to demonstrate the end-to-end modeling workflow without exposing confidential business data. The project preserves the original PAM (Gower distance) clustering and per-cluster prediction framework while adapting the preprocessing pipeline and Shiny UI application to fit the new dataset structure.
